...
public class WatchDir1 {
	...
	WatchDir1(Path dir, boolean recursive) throws IOException {
		...
		if(recursive){
			System.out.println("Scanning all start " + dir);
			registerAll(dir);
	...
	void processEvents(){
		for(;;){
			...
			for(WatchEvent<?> event: key.pollEvents()){
				...
				//if dir is created, and watching recursively, then register it and its sub-directories
	...
	public static void main(String[] args) throws IOException {
		if(args.length != 2 && args.length != 1) usage();
        ...
        Path userDir = LocalPath.USER_DIR.path();
        
        System.out.println(userDir.resolve(dir));
        new WatchDir1(userDir.resolve(dir), recursive).processEvents();
	}
}
